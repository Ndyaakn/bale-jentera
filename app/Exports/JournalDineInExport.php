...
class JournalDineInExport implements FromView, WithTitle
{
    use Exportable;
    ...
    public function view(): View 
    {
         $data = FinancialStatementDescription::with('financial_statement_title', 'order.order_menus')->whereHas('order', function($q){
             $q->where([['order_category', 'dine in'], ['order_status', 'pay']]);
         })
         ...
         {  
            ...
            $journal_4 = ['date' => $item->financial_statement_title->date, 'price' => $item->debit, 'menu' => $journal_2, 'jumlah_menu' => $journal_3, 'harga_menu' => $journal_4, 'rowspan' => count($journal_2), 'tipe_pembayaran' => $item->order->pembayaran, 'customer' => $item->order->customer_name];
            array_push($journal_1, $journal_4);
         }
        //  dd($journal_1);
         $journal['journal'] = $journal_1;
         $journal['total'] = $data->sum('debit');
        return view('excel.journal_dinein', $journal);
    }
